---
title: "Debugging code: in-class exercises - SOLUTIONS"
...
babynames %>%
  ...
  ggplot(mapping = (aes(x = year, y = distinct_names))) + 
  geom_line() + 
  labs(
      title = "Disinct names over time",
      x = "Year",
...
# Call the function with the name "Sabrina"
name_trend(babynames, "Sabrina")
```

Explanation of tidy eval error:
` {{}}` is necessary when you are referring to columns by names stored in variables, not to their actual values. Here we are passing the actual value (the name "Sabrina") directly to the filter function, we are not passing a variable that contains a column name. Therefore, ` {{}}` is not necessary.
